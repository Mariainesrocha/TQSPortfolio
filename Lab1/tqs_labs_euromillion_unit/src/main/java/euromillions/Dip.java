...
public class Dip {
    ...
    // 2c
    private static int MAX_OF_STARS = 2;
    private static int MAX_OF_NUMS = 5;

    private static int MAX_RANGE_STARS = 8;
    private static int MAX_RANGE_NUMS = 49;
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        if (MAX_OF_NUMS == arrayOfNumbers.length && MAX_OF_STARS == arrayOfStarts.length) {
    ...
    public static Dip generateRandomDip() {
        ...
        for (int i = 0; i < MAX_OF_NUMS; ) {
            int candidate = generator.nextInt(MAX_RANGE_NUMS) + 1;
            if (!randomDip.getNumbersColl().contains(candidate)) {
                randomDip.getNumbersColl().add(candidate);
                i++;
            }
        }
        for (int i = 0; i < MAX_OF_STARS; ) {
    ...
    public String format() {
        ...
        for (int star : getStarsColl()) {
            sb.append(String.format("%3d", star)); //** 2a
        }
        sb.append("]"); // ** 2a
        return sb.toString();
    }
